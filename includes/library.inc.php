...
function show_my_movies() {
    ...
    if (empty($search_term)) {
        ...
        $query = "SELECT * FROM library_master_movies WHERE title <> '' AND title LIKE \"%$search_term%\" ORDER BY created DESC LIMIT $start,$end ";
    }
...
function show_my_shows() {
    ...
    if (empty($search_term)) {
        ...
        $query = "SELECT * FROM library_master_shows WHERE title <> '' ORDER BY created DESC LIMIT $start,$end ";
    } else {
        $topt['num_table_rows'] = $db->qSingle("SELECT COUNT(*) FROM library_master_shows WHERE title <> '' AND title LIKE \"%$search_term%\" ");
        $query = "SELECT * FROM library_master_shows WHERE title <> '' AND title LIKE \"%$search_term%\" ORDER BY created DESC LIMIT $start,$end ";
    }
    $results = $db->query($query);
